...
#   계열 : A (경로/IP 치환)
#   단계 : A-1  전수조사
#   역할 : AS-IS 하드코딩 경로(d:/)를 텍스트·class·아카이브 전부에서 찾아 리포트
...
#     (5) 출력 폴더 자동 생성
# v8.1: (1) Out 기본값 report\ 폴더로 (자동 생성, 영어 폴더명)
#       (2) 백업 폴더 자동제외 정규식 수정 — v8은 <이름>_backup_path_* 형태를 못 잡았음
# v8.2: 단일 모드 기본 리포트명에도 소스명 접두 (report\<소스명>_asis_path_report.dat)
#       — 일괄 모드와 규칙 통일, 여러 소스 단일 실행 시 덮어쓰기 방지. -Out 지정 시엔 그 이름 그대로
#
...
function Test-Excluded([string]$path) {
    ...
    if ($path -match '[\\/][^\\/]*_backup_(path|ip)_') { return $true }   # <이름>_backup_path_* 자동 제외 (v8.1 수정)
...
Write-Host "===== Find-AsisPath v8.2 (Scope=$Scope) ====="
...
if ($RootList) {
    ...
    # v8.2: 단일 모드도 기본 파일명엔 소스명 접두 (일괄 모드와 일치, 소스 30개 덮어쓰기 방지)
    #       -Out으로 파일명을 직접 지정하면 그 이름 그대로 사용
    $outFile = $Out
