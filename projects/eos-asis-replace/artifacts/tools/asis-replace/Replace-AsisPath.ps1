...
param(
    ...
    [string]$DetectPattern = "(?<![a-zA-Z0-9])[A-Za-z]:[/\\]|(?<![\w:])\\{2,4}(?!(?:x[0-9a-fA-F]{2}|u[0-9a-fA-F]{4})(?![\w\-]))(?=[A-Za-z0-9][\w\-]*(?:\.[\w\-]+|(?:\\{1,2}|/)[^\s`"'<>|*?:,;=(){}\[\]\\/]{2,}))|(?<=[`"'])\\{2,4}(?!(?:x[0-9a-fA-F]{2}|u[0-9a-fA-F]{4})(?![\w\-]))(?=[A-Za-z0-9][\w\-]*[A-Za-z0-9][`"'])",
